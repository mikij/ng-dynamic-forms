...
export type DynamicFormModelMapFn = (
  type: string,
  model: any,
  layout: any
) => DynamicFormControlModel | null;

export const DYNAMIC_FORM_MODEL_MAP_FN =
  new InjectionToken<DynamicFormModelMapFn>("DYNAMIC_FORM_MODEL_MAP_FN");
...
export class DynamicFormService {
  constructor(
    ...
    @Inject(DYNAMIC_FORM_MODEL_MAP_FN)
    @Optional()
    private readonly DYNAMIC_FORM_MODEL_MAP_FN: any
  ) {
    this.DYNAMIC_FORM_MODEL_MAP_FN =
      DYNAMIC_FORM_MODEL_MAP_FN as DynamicFormModelMapFn;
  }
      ...
      switch (model.type) {
        case DYNAMIC_FORM_CONTROL_TYPE_ARRAY:
          ...
          formArrayModel.groupFactory = () =>
            this.fromJSON(formArrayModel.groupPrototype);
        ...
        default:
          {
            if (this.DYNAMIC_FORM_MODEL_MAP_FN !== undefined) {
              var model = this.DYNAMIC_FORM_MODEL_MAP_FN(
                model.type,
                model,
                layout
              );
              ...
              if (model !== null) {
                ...
                throw new Error(
                  `custom form control model is not resolved by type "${model.type}"`
                );
              }
            } else {
              throw new Error(
